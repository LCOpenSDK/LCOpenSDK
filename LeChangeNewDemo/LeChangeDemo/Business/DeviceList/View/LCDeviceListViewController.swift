...
@objcMembers class LCDeviceListViewController : LCBasicViewController {
    ...
    private func addAction() {
        let addChooseView =  LCAddDeviceChooseView()
            addChooseView.resultBlock = { [weak self]type in
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    if type == .scan {
                        ...
                        //跳转自发现 - 蓝牙搜索设备
                        self?.navigationController?.pushToBluetoothSearchPage()
                    }
                }
            }
        addChooseView.show(offsetY: LC_statusBarHeight + lc_navBarHeight)
    }
