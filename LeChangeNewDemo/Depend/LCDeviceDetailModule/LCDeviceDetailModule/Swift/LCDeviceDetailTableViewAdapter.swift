    ...
    init(isMultipleChannels: Bool = false, deviceInfo: LCDeviceInfo, selectedChannelId: String, presenter: LCDeviceDetailPresenter? = nil) {
        ...
        if isMultipleChannels {
            ...
            self.cellsModel = [
                ...
                [LCTableViewCellModel(icon: UIImage(named: "icon_network_configuration"), title: "mobile_common_network_config".lc_T, subTitle: "", height: 70, showArrow: true, canusing: self.isOnline(), selectAction: { [weak self] in
                    self?.presenter?.pushToWifiSettings(deviceId: self?.deviceInfo.deviceId ?? "")
                })
                ]
            ]
        }
