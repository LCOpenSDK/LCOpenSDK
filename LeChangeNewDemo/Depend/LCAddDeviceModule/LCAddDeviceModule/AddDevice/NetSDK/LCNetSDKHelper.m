...
+ (void)logoutDevice:(long)loginHandle completion:(dispatch_block_t)completion {
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		[LCNetSDKInterface logout:loginHandle];
		dispatch_async(dispatch_get_main_queue(), ^{
			if (completion) {
				completion();
			}
...
+ (void)queryProductDefinition:(long)loginHandle
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		LCDeviceProductDefinition *definition = [LCNetSDKInterface queryProductDefinitionInfo:loginHandle];
		dispatch_async(dispatch_get_main_queue(), ^{
			if (definition) {
				success(definition);
			} else {
...
+ (void)queryDeviceUserInfoDefinition:(long)loginHandle
                              ...
                              failure:(dispatch_block_t)failure {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        LCDeviceUserInfoDefinition *definition = [LCNetSDKInterface queryDeviceUserInfo:loginHandle];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (definition) {
                success(definition);
            } else {
...
+ (void)queryPasswordResetType:(LCDeviceNetInfo *)device
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        LCDeviceResetPWDInfo * Info = [LCNetSDKInterface queryPasswordResetType:device byPhoneIp:phoneIp];
		dispatch_async(dispatch_get_main_queue(), ^{
			result(Info);
		});
	});
...
+ (void)resetPassword:(NSString *)password
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
		LCDevicePWDResetInfo *info = [LCNetSDKInterface resetPassword:password device:device securityCode:securityCode contact:contact useAsPreset:useAsPreset byPhoneIp:phoneIp];
		dispatch_async(dispatch_get_main_queue(), ^{
			result(info);
		});
	});
...
{
    [LCNetSDKInterface startNetSDKReportByRequestId:requestId];
    
}

+ (void)stopNetSDKReport{
    [LCNetSDKInterface stopNetSDKReport];
}
@end
