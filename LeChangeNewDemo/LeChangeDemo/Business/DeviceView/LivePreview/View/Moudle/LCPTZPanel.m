...
- (instancetype) initWithFrame:(CGRect)frame {
    @throw [NSException exceptionWithName:@"LCPTZPanel init error" reason:@"LCPTZPanel must be initialized with a style and a mode. Use 'initWithFrame:style:' instead." userInfo:nil];
    return [self initWithFrame:frame style:LCPTZPanelStyle4Direction];
}
...
- (void)loadUI:(CGRect)frame
{
    ...
    if (mPTZStyle==LCPTZPanelStyle8Direction) {
        // TODO:添加8方向图片
    }
...
- (VPDirection)calcDirction:(CGPoint)translation
{
    ...
    {
        if(mPTZStyle == LCPTZPanelStyle4Direction) return VPDirectionUnknown;
...
- (void)showHighlight:(VPDirection)ptzDirection
{
    ...
    if(isLandscape && mPTZStyle == LCPTZPanelStyle4Direction) {
    ...
    switch (ptzDirection)
    {
        ...
        {
            if(mPTZStyle == LCPTZPanelStyle4Direction) break;
...
- (void)updatePanelBackguoundImageWithPT1:(BOOL)hasPT1 {
    if (hasPT1) {
        ...
        mPTZStyle = LCPTZPanelStyle4Direction;
    }
