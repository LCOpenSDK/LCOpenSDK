...
extension LCDeviceDetailPresenter {
    ...
    func pushToWifiSettings(deviceId: String) {
        /**
         跳转设置网络
        ...
        if let vc = LCRouter.object(forURL: "/lechange/adddevice/onlineWifiConfig", withUserInfo: ["deviceId": deviceId]) as? UIViewController {
            self.viewController?.navigationController?.pushViewController(vc, animated: true)
        }
    }
}
